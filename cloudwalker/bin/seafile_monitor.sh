...
log_file=/var/log/seafile_monitor.log
...
for fs in $stores ; do 
	...
	if [ $? -ne 0 ] ; then
		echo "WARNING: $fs not mounted, restarting seafile" >> $log_file
		seafile_restart
	fi
	...
	if [ $? -ne 0 ] ; then
		echo "WARNING: seahub process has stopped, restarting seafile" >> $log_file
		seafile_restart
	fi
	pid_ccnet=$(cat /opt/seafile/pids/ccnet.pid)
	ps -p $pid_ccnet -o cmd=  | grep 'ccnet-server' >/dev/null 2>&1
	if [ $? -ne 0 ] ; then
		echo "WARNING: ccnet process has stopped, restarting seafile" >> $log_file
		seafile_restart
	fi
	pid_seafile=$(cat /opt/seafile/pids/seaf-server.pid)
	ps -p $pid_seafile -o cmd=  | grep 'seaf-server' >/dev/null 2>&1
	if [ $? -ne 0 ] ; then
		echo "WARNING: seaf-server process has stopped, restarting seafile" >> $log_file
		seafile_restart
	fi
done
